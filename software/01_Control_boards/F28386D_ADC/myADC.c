							...
							uint32_t signalMode)
{
    ...
    // SOC1 konfigurieren:
    // Messung durch das ePWM8-Modul triggern (SOCA)
    AdcaRegs.ADCSOC1CTL.bit.TRIGSEL = ADC_TRIGGER_EPWM8_SOCA;
    // Eingang ADCIN1 setzen
    AdcaRegs.ADCSOC1CTL.bit.CHSEL = ADC_SINGLE_ENDED_ADCIN1;
    // Abtastzeitfenster 60 (SYSCLK-)Taktzyklen = 300 ns
    AdcaRegs.ADCSOC1CTL.bit.ACQPS = 59;

    // SOC2 konfigurieren:
    // Messung durch das ePWM8-Modul triggern (SOCA)
    AdcaRegs.ADCSOC2CTL.bit.TRIGSEL = ADC_TRIGGER_EPWM8_SOCA;
    // Eingang ADCIN2 setzen
    AdcaRegs.ADCSOC2CTL.bit.CHSEL = ADC_SINGLE_ENDED_ADCIN2;
    // Abtastzeitfenster 60 (SYSCLK-)Taktzyklen = 300 ns
    AdcaRegs.ADCSOC2CTL.bit.ACQPS = 59;
    ...
    // ausgelöst wird, wenn alle drei Messwerte vorliegen
		...
		AdcaRegs.ADCINTSEL1N2.bit.INT1SEL = ADC_EOC_NUMBER_2;
...
{
		// LED D1002 auf dem ControlBoard zur Visualisierung blinken lassen
		GpioDataRegs.GPATOGGLE.bit.GPIO5 = 1;

		// Aktuelle Messwerte auslesen
		ADCIN0 = AdcaResultRegs.ADCRESULT0;
		ADCIN1 = AdcaResultRegs.ADCRESULT1;
		ADCIN2 = AdcaResultRegs.ADCRESULT2;
